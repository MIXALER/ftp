...
//		printf("fd=%d,p->d_name=%s\n",fd,p->d_name);
        ...
        memset(&t, 0, sizeof(t));
        sprintf(t.buf, "%-20s %c %15ldB          %04d-%02d-%02d %02d:%02d", p->d_name, pt(p->d_type), buf.st_size,
                p1->tm_year + 1900, p1->tm_mon + 1, p1->tm_mday, p1->tm_hour, p1->tm_min);
        t.len = strlen(t.buf);
